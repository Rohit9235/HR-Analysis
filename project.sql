...
desc `hr_2 second`;
show tables;
select * from `hr_1 second`;
select * from `hr_2 second`;
...
then 1
Else 0
End as attrition_y from `hr_1 second` ) as a
...
end as attrition_rate from `hr_1 second`) as a
inner join `hr_2 second` as b on b.EmployeeID = a.EmployeeNumber
group by a.department;

/* 4-- Average Working Years for Each Department -- */
select a.department, format(avg(b.TotalWorkingYears),1) as Average_Working_Year
from `hr_1 second` as a
inner join `hr_2 second` as b on b.Employeeid=a.EmployeeNumber
group by a.department;
